...
func NewFromFile(fileName string, fileEncoding string) (table *DbfTable, err error) {
	...
	return NewFromBytes(s, fileEncoding)
}

func NewFromUrl(url string, fileEncoding string) (table *DbfTable, err error) {
	s, err := readUrl(url)
...
func NewFromBytes(s []byte, fileEncoding string) (table *DbfTable, err error) {
